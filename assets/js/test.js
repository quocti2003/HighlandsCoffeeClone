...
const allStoresSimplified = allStores.map(store => {
  // Lặp qua từng cửa hàng (store) trong mảng allStores.
  // Hàm map() sẽ tạo ra một mảng mới dựa trên kết quả trả về của hàm này cho mỗi phần tử.
  return {
    // Tạo một object mới chỉ chứa thuộc tính 'name' và 'address' từ đối tượng 'store' ban đầu.
    name: store.name, // Lấy tên cửa hàng
    address: store.address // Lấy địa chỉ cửa hàng
    // Bỏ qua các thuộc tính 'lat' và 'lng'.
  };
});

console.log(allStoresSimplified); // In ra mảng mới 'allStoresSimplified' chứa thông tin đã được đơn giản hóa.